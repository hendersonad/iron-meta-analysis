...
load_brms_fits <- function(input_data){
  ...
  fit_name <- paste0("brmsfits/fairhf2_set3/", name, "_0.05.rds")
...
ggplot(all_bayes_trt_effects, aes(x = avg_effect, y =forcats::fct_rev(outcome), fill = after_stat(x < 1))) + 
  ...
  labs(y = "", x = "Posterior distribution for average RR/HR", caption = bquote(tau ~ scale ~ prior == 0.05)) +
