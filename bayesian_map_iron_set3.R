...
iron_tte_cnpt <- iron_tte_cnpt |> filter(trial %in% c("IRONMAN", "HEART-FID", "FAIR-HF2"))
iron_tte_cvd <- iron_tte_cvd |> filter(trial %in% c("IRONMAN", "HEART-FID", "FAIR-HF2"))
iron_tte_acm <- iron_tte_acm |> filter(trial %in% c("IRONMAN", "HEART-FID", "FAIR-HF2"))
...
ggsave(all_estimates, filename = here::here("output/fairhf2_set3/fig1_trial_estimates.pdf"), width = 9  , height = 4, units = "in")
...
do_frequentist_style <- function(
  ...
  filename = here::here("output/fairhf2_set3", paste0("frequentist_meta_", deparse(substitute(input_data)), ".pdf"))
...
do_ranef_brms <- function(dataset = iron_data, tauprior = 0.5, savename = "temp"){
  ...
  fit_name <- paste0("brmsfits/fairhf2_set3/", savename, "_", tauprior)
  brm(
    random_model,
    dataset,
    prior = random_prior, 
...
ggsave(plot_rec_cnpt[[8]], filename = here::here("output/fairhf2_set3/fig2a_forest_rec_cnpt.pdf"), width = 3, height = 4, units = "in")
ggsave(plot_rec_hfh[[8]], filename = here::here("output/fairhf2_set3/fig2b_forest_rec_hfh.pdf"), width = 3, height = 4, units = "in")
ggsave(plot_tte_cnpt[[8]], filename = here::here("output/fairhf2_set3/fig2c_forest_tte_cnpt.pdf"), width = 3, height = 4, units = "in")
ggsave(plot_tte_cvd[[8]], filename = here::here("output/fairhf2_set3/fig2d_forest_tte_cvd.pdf"), width = 3, height = 4, units = "in")
ggsave(plot_tte_acm[[8]], filename = here::here("output/fairhf2_set3/fig2e_forest_tte_acm.pdf"), width = 3, height = 4, units = "in")

# for abstract submission
ggsave(plot_rec_cnpt[[8]], filename = here::here("output/fairhf2_set3/iron_abstract_combined_bigchronictrials.jpeg"), width = 820*0.5 , height = 1080*0.5, units = "px", scale = 2)
...
bayes_gt |> 
  gt::gtsave(here::here("output/fairhf2_set3/table2_bayesian_results.docx"))
